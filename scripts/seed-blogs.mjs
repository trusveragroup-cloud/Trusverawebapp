...
const posts = [
  ...
  },
  {
    ...
    author: "Priya Desai",
    author_role: "Senior Demand Strategist",
    author_initials: "PD",
  ...
  },
  {
    ...
    author: "Sneha Joshi",
    author_role: "Email Marketing Lead",
    author_initials: "SJ",
    read_time: "5 min read",
    cover_image: "https://images.unsplash.com/photo-1596526131083-e8c633064d28?w=1200&q=80",
    featured: false,
...
async function seed() {
  ...
  for (const post of posts) {
    const { error } = await supabase.from("blogs").upsert(post, { onConflict: "slug" })
    if (error) console.error("Error seeding:", post.slug, error)
