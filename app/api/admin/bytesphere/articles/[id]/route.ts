...
export async function PUT(
  ...
  try {
    ...
    if (featured) {
      await supabase.from("bs_articles").update({ featured: false }).eq("featured", true).neq("id", id)
    }
    ...
    return NextResponse.json({
      success: true,
      message: status === "Published" ? "Article updated and published." : "Article saved.",
      article,
    })
  } catch (err) {
...
export async function DELETE(
  ...
  try {
    ...
    const { error } = await supabase.from("bs_articles").delete().eq("id", id)
