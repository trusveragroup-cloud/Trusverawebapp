...
export default function EditBlogPostPage() {
  ...
  const fetchPost = useCallback(async () => {
    ...
    try {
      ...
      const post: BlogRow = data.blogs[0]
      setTitle(post.title)
      setSlug(post.slug)
      ...
      setAuthorInitials(post.author_initials)
      ...
      pendingContent.current = post.content
    } catch {
      setPageError("Failed to load post. Please try again.")
    } finally {
  ...
  const savePost = async (postStatus: "Draft" | "Published") => {
    ...
    if (!excerpt.trim()) { alert("Please add an excerpt."); return }
    if (!category) { alert("Please select a category."); return }
    if (!author.trim()) { alert("Please add an author."); return }
  ...
  return (
    <div style={{ padding: 32 }}>
      ...
      {!pageLoading && pageError && (
        <div style={{ padding: 100, textAlign: "center" }}>
          <AlertCircle size={32} color={C.red400} style={{ marginBottom: 12 }} />
            ...
            <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
              {saveError && (
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <AlertCircle size={14} color={C.red400} />
                  <span style={{ fontFamily: "var(--font-inter)", fontSize: 12, color: C.red400 }}>
                    {saveError}
                  </span>
                </div>
              )}
