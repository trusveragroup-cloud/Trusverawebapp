...
import { Zap, Clock, ChartPie, Handshake, type LucideIcon } from "lucide-react";
import { C } from "@/lib/colors";
import { PROFILING_BENEFITS } from "@/lib/data";
...
export default function WhyProfilingMatters() {
  return (
    <section style={{ background: C.cream100, padding: "120px 24px" }}>
      ...
      >
        <h2 style={{ fontFamily: "var(--font-dm-serif)", fontSize: 34, color: C.textDark, fontWeight: 400 }}>
          Why Account Intelligence Wins Deals
        </h2>
      </motion.div>

      <motion.div
        className="ap-benefits-grid"
        initial="hidden"
        whileInView="visible"
        viewport={{ once: true, amount: 0.1 }}
        variants={container}
      ...
      >
        {PROFILING_BENEFITS.map((benefit) => {
